...
public class PaymentServiceImpl implements OpenService {
	...
	private void installWechatPay() {
		...
		Configure.setCertLocalPath(certs_path
				.concat(PayParam.PAY_WECHAT_CERT_PATH_APP.getValue()));
		...
		Configure.setCertLocalPath_app(certs_path
				.concat(PayParam.PAY_WECHAT_CERT_PATH_APP.getValue()));
		Configure.setCertPassword_app(PayParam.PAY_WECHAT_CERT_PWD_APP
				.getValue());
	...
	private void installTenpay() {
		String host = PayParam.PAY_HOST.getValue();
		String certs_path = PayParam.PAY_CERTS_PATH.getValue();
		TenpayConfig.partner = PayParam.PAY_TENPAY_PARTNER.getValue();
		TenpayConfig.key = PayParam.PAY_TENPAY_KEY.getValue();
		TenpayConfig.cert_path = certs_path
				.concat(PayParam.PAY_TENPAY_CERT_PATH.getValue());
		TenpayConfig.cert_pwd = PayParam.PAY_TENPAY_CERT_PWD.getValue();
		TenpayConfig.web_notify_url = host
				.concat("/tenpay/web/payment_back");
		TenpayConfig.web_return_url = host
				.concat("/tenpay/web/payment_front");
		TenpayConfig.wap_partner = PayParam.PAY_TENPAY_WAP_PARTNER.getValue();
		TenpayConfig.wap_key = PayParam.PAY_TENPAY_WAP_KEY.getValue();
		TenpayConfig.wap_cert_path = certs_path
				.concat(PayParam.PAY_TENPAY_WAP_CERT_PATH.getValue());
		TenpayConfig.wap_cert_pwd = PayParam.PAY_TENPAY_WAP_CERT_PWD.getValue();
		TenpayConfig.wap_notify_url = host
				.concat("/tenpay/wap/payment_back");
		TenpayConfig.wap_return_url = host
				.concat("/tenpay/wap/payment_front");
	}
	...
	private void installChinapay() {
		...
		ChinapayConfig.wap_frontUrl = host
				.concat("/chinapay/wap/front_payment");
		ChinapayConfig.wap_backUrl = host.concat("/chinapay/wap/back_payment");
		ChinapayConfig.app_backUrl = host.concat("/chinapay/app/back_payment");
	}
	...
	private void installAlipay() {
		...
		AlipayConfig.web_payment_return = host
				.concat("/alipay/web/return_payment");
		AlipayConfig.web_payment_notify = host
				.concat("/alipay/web/notify_payment");
		AlipayConfig.refundment_notify = host
				.concat("/alipay/web/notify_refundment");
		// wap参数初始化
		AlipayConfig.private_key = PayParam.PAY_ALIPAY_PRIVATE_KEY.getValue();
		AlipayConfig.wap_public_key = PayParam.PAY_ALIPAY_WAP_PUBLIC_KEY
				.getValue();
		AlipayConfig.wap_payment_return = host
				.concat("/alipay/wap/return_payment");
		AlipayConfig.wap_payment_notify = host
				.concat("/alipay/wap/notify_payment");
		// app参数
		AlipayConfig.app_public_key = PayParam.PAY_ALIPAY_APP_PUBLIC_KEY
				.getValue();
		AlipayConfig.app_payment_notify = host
				.concat("/alipay/app/notify_payment");
	}
