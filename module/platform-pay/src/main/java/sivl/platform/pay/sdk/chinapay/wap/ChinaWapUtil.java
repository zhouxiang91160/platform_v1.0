package sivl.platform.pay.sdk.chinapay.wap;
...
public class ChinaWapUtil {
	...
	public static ResultModel<Object> payment(PaymentsModel payment) {
		...
		try {
			...
			requestData.put("frontUrl", ChinapayConfig.wap_frontUrl);

			// 5.后台通知地址如果上送了带有？的参数，例如：http://abc/web?a=b&c=d
			// 在后台通知处理程序验证签名之前需要编写逻辑将这些字段去掉再验签，否则将会验签失败
			 requestData.put("backUrl", ChinapayConfig.wap_backUrl);
