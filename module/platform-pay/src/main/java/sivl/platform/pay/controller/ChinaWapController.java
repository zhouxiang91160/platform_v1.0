...
@Controller
@RequestMapping("/chinapay/wap")
public class ChinaWapController extends PaymentController{
	
	private static String className = "sivl.platform.pay.controller.ChinaWapController";
	...
	public void returnUrl(HttpServletResponse response,
		...
		LogUtil.netLegalLog(
				JSONUtil.obj2json(new NetLogModel(request)
						...
						.buildInterfaceName("/chinapay/wap/front_payment")
						.buildMsg("银联卡支付前台通知")), ChinaWapController.class);
	...
	public void notifyUrl(HttpServletResponse response,
		...
		LogUtil.netLegalLog(
				JSONUtil.obj2json(new NetLogModel(request)
						.buildClassName(className)
						.buildInterfaceName("/chinapay/wap/back_payment")
						.buildMsg("银联卡支付后台通知")), ChinaWapController.class);
		PaymentsModel payment = new PaymentsModel();
		payment.setRequest(request);
		ResultModel<Object> result = ChinaWebUtil.paymentVerify(payment);
		if (result.getCode().equals(ResultCons.SUCCESS)) {
